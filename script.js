...
if(tercioSection){
  ...
  const routes={URCE:'#demonstrators',GADIR:'gadir.html',ATALA:'#demonstrators',ALANO:'#demonstrators',ARDID:'#demonstrators'};
  tercioSection.querySelectorAll('.layer-card').forEach(card=>{
    ...
    if(name!=='GADIR')link.setAttribute('aria-label',`${name} capability — detailed page in development`);
    card.replaceWith(link);link.append(card);
    const hint=document.createElement('span');hint.className='card-explore';hint.textContent=name==='GADIR'?'Explore system →':'Capability page in development';card.append(hint);
  });
