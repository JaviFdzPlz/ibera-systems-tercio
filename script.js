...
if(tercioSection){
  ...
  const routes={URCE:'#demonstrators',GADIR:'gadir.html',ATALA:'atala.html',ALANO:'#demonstrators',ARDID:'#demonstrators'};
  tercioSection.querySelectorAll('.layer-card').forEach(card=>{
    ...
    if(!['GADIR','ATALA'].includes(name))link.setAttribute('aria-label',`${name} capability — detailed page in development`);
    card.replaceWith(link);link.append(card);
    const hint=document.createElement('span');hint.className='card-explore';hint.textContent=['GADIR','ATALA'].includes(name)?'Explore system →':'Capability page in development';card.append(hint);
  });
